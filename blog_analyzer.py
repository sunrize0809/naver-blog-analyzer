...
with st.sidebar:
    st.header("⚙️ 설정")
    api_key_input = st.text_input("Google AI API Key 입력", type="password")
    blog_id_input = st.text_input("분석할 네이버 블로그 ID", value="doctordk")
    
    # 3번 요청사항: URL 수집량 증가를 위한 슬라이더 (최대 10페이지 = 약 300개 글)
...
if start_button:
    if not api_key_input:
        st.warning("👈 왼쪽 설정 메뉴에서 Google API Key를 입력해 주세요.")
    elif not blog_id_input:
